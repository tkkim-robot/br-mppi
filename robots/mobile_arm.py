...
class MobileArmRobot(RobotModel):
    ...
    def __init__(self) -> None:
        ...
        self.link_widths = np.array([[0.2, 0.15, 0.1, 0.05], [0.2, 0.15, 0.1, 0.2]], dtype=float)
    ...
    def body_points(self, state: np.ndarray) -> np.ndarray:
        ...
        x, y, theta = state[:3]
        rot = self._rot(theta)
        points = [base @ rot.T + np.array([x, y])]
    ...
    def draw(self, ax, state: np.ndarray, **kwargs) -> None:
        ...
        points = self.body_points(state)
        ax.add_patch(Polygon(points[:4], closed=True, facecolor=color, edgecolor=edgecolor, alpha=0.55, **kwargs))
        for polygon in self.link_polygons(state):
            ax.add_patch(
