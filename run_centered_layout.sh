#!/bin/bash
mkdir -p "output/hero/final_centered_layout"
...
for pair in "${PAIRS[@]}"; do
  ...
  # We use algos explicitly to match the 5 algorithms used in clean_final to ensure there are exactly 4 small plots
  uv run python examples/hero_benchmark.py \
    ...
    --save-video \
    --headless \
    --output-dir "output/hero/final_centered_layout"
    
  echo "Finished $robot!"
done

echo "ALL CENTERED RENDERS COMPLETE!"
